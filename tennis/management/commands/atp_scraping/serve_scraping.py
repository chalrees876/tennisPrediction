...
def main():
    ...
    for row in rows:
        ...
        for i, column in enumerate(columns):
            if i == 1:
                ...
                df = column.text.strip()
            elif i == 8:
                a = column.text.strip()
            else:
                continue
        player_stats_dict = dict(zip(header_list, [name, serve_rating, fsp, fspw, sspw, sgwp, df, a]))
    ...
    player_stats.coalesce(1).write.csv('./data/separate_data/serve_data',
